...
const LogdingCategory = () => {
  const boxes: JSX.Element[] = [];
  lodgingTypes.map((item, idx) => {
    boxes.push(
      <button key={idx} className="p-[10px] flex flex-col items-center gap-[5px] text-[12px] h-[100%] w-[100%]">
        {item.icon}
  ...
  return (
    <div>
      <SimpleSlider itemArray={boxes} />
    </div>
  )
}

export default LogdingCategory